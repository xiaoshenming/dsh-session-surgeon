...
同时能修好打不开的 DeepSeek Harness 会话（seq gap / torn zstd / 孤立代理字符 / 消息缺 ID）。`inspect` 还会**警告**悬空 `tool/call`（没有对应 `tool/result`，下次模型请求永久 400），但**不会编**假结果。官方以后修加载器，也救不回已经坏掉的 `session.jsonl.zstd`。
